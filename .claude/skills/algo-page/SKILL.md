...
- [ ] Pipeline figures use the canonical generator pattern when they have > 4 stages: a matplotlib snake-layout SVG modeled on `py/generate_gp_checkerboard_pipeline.py` (slate-50 figure card, white axes, `FancyBboxPatch` boxes, 12-pt labels, `svg.hashsalt` deterministic output, accessibility post-pass). **Never use `flowchart LR` for > 4 stages** — it overflows narrow viewports. ≤ 4 stages may stay inline as `flowchart TB` (Mermaid). A linear pipeline figure that parallels the `:::algorithm[...]` procedure IS valuable when it encodes the visual flow (snake wrap, parallel paths, fan-out, feedback) at a glance — keep it. Drop only if every stage is a literal one-line restatement of the numbered step AND no spatial information is added.
