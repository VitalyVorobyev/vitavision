...
- [ ] Mermaid pipelines obey the hard rules in `_shared/illustrations.md`: **`flowchart LR` is forbidden when the pipeline has > 4 stages** (it overflows the content column on narrow viewports) — use `flowchart TB`, a generated SVG, or drop the figure. **Never restate the `:::algorithm[...]` procedure** as a Mermaid pipeline; a linear N-stage chain that mirrors the numbered steps is redundant noise — delete it. A pipeline figure earns its place only when it encodes branching, parallelism, fan-out, or a feedback loop the procedure cannot show.
