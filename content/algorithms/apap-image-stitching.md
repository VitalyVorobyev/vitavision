...
- Supersedes the two-plane dual-homography warp (Gao et al., historical): APAP's continuous grid of per-cell homographies subsumes the two-plane parametrisation as a special case.
- Generalises the as-similar-as-possible warp (Lin SVA): APAP's projective per-cell grid is strictly more general, though the affine deviation field remains a useful baseline for moderate-parallax planar-scene panoramas.
