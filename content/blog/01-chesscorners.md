...
(The old OpenCV `findChessboardCorners` chessboard detector follows a different route: it starts from segmenting black squares and then derives corners from the recovered geometry. That works well, but couples the problem to the global board structure. The newer `findChessboardCornersSB` detector is based on [the Duda-Frese local X-junction detector](/atlas/duda-radon-corners), which is a solid alternative to the approach we discuss here. Both detectors expect the full requested pattern to be visible, which is not acceptyble in my case.)
