...
The [previous post](/blog/01-chesscorners) showed how to detect chessboard corners with [ChESS response](/atlas/chess-corners). That gives a sparse set of X-junctions, each with two local grid axes. Some true corners may be missing, and some detections may be false positives.

This post starts from that output and focuses on **grid reconstruction**: recovering grid connectivity and integer coordinates from detected corner candidates.

The setups I care about involve partially visible or occluded boards with strong lens distortion. Rather than detecting a predefined board directly from the image, I want to reconstruct whatever grid structure is supported by the detected corners. Strong distortion also makes global grid lines or a single homography poor choices as the primary reconstruction model.

For this reconstruction step, I use a topological approach based on the [Topological Grid Finding](/atlas/shu-topological-grid) method. I implemented it in the [projective-grid](https://crates.io/crates/projective-grid) crate. This post describes the main steps of that implementation.

The method starts locally. It connects nearby corners into plausible cells, then uses grid topology to remove inconsistent detections and assign integer coordinates. Geometric refinement can still be applied later, after the structure is known.

We will use this image throughout the post to illustrate the steps.

![](../images/02-topo-grid/GeminiChess1/00-input.png)

# 1. Grid as a graph

We start from a set of candidate X-junctions like the ones shown below. I use [chess-corners](https://crates.io/crates/chess-corners), which provides local grid orientations for each corner.

![](../images/02-topo-grid/GeminiChess1/01-corners-axes.png)

To recover the grid, we need to determine which corners are neighbors and which groups of four form grid cells. We represent this as a graph:
...
Following [Shu, Brunton, and Fiala](/atlas/shu-topological-grid), we build the initial graph using Delaunay triangulation of the detected corners.
...
For a regular grid, the triangulation usually contains the true neighbor connections plus one diagonal inside each grid cell. On the test image, this gives:

![](../images/02-topo-grid/GeminiChess1/03-delaunay-edge-kinds.png)

The graph is only an initial hypothesis: it may contain incorrect edges and connections to false detections. The following steps classify these edges and recover quadrilateral grid cells.

Importantly, this construction is local. It does not assume that grid rows project to straight image lines, which makes it suitable for strongly distorted images.
...
The overlay below shows triangle pairs with classified edges. For the test image, they form the complete correct set of quadrilateral cells.
...
My implementation adds two weak geometric checks:
...
The ordering is propagated through the mesh rather than fitted image lines. A grid row may appear strongly curved under lens distortion, but its order in the mesh remains unchanged.
...
The grid reconstruction described above is used as the core of the chessboard detector in [`calib-targets`](https://crates.io/crates/calib-targets). The crate provides a high-level API that runs the complete pipeline, from an input image to an ordered set of chessboard corners.
...
I omitted two implementation details that matter in difficult cases: growing a grid component with detected corners that were not assigned to any cell, and merging nearby disconnected components.
...
The reconstruction stage itself is not limited to plain chessboards. The [`calib-targets`](https://crates.io/crates/calib-targets) crate uses the same topological backbone for several target detectors for chessboard, ChArUco, and [PuzzleBoard](/atlas/puzzleboard).

PuzzleBoard is a relatively new target type. I think this is the most interesting self-localizing square grid target proposed so far. I am going to discuss it in more detail in a future post.

The [projective-grid](https://crates.io/crates/projective-grid) crate also supports hexagonal grids, which are outside the scope of this post. It is self-contained and can be used independently of calibration targets.

# Conclusion

The key advantage of this approach is that grid reconstruction is largely independent of the calibration target itself. Given corner positions and local grid orientations, the same topological core can reconstruct the grid for plain chessboards, ChArUco, and PuzzleBoard targets.

It does not require a known board size, straight projected grid lines, or a global homography. This makes the same reconstruction machinery useful for partial and occluded views and under strong lens distortion, while remaining fast enough for online detection.

The main geometric limitation comes from Delaunay triangulation itself: it is not projective invariant. At very oblique viewing angles, it can connect points that are not real grid neighbors.
