...
- C1: A local-inference pipeline — "FocalClick is the first pipeline that deals with interactive segmentation from a completely local view, which reaches SOTA with significantly smaller FLOPs." (§1, "In general, our contributions could be summarized as follows" enumeration, item 1)
- C2: Interactive Mask Correction benchmark + Progressive Merge solution — "We introduce the first benchmark and sub-task to evaluate the ability to correct preexisting masks, and propose corresponding solutions." (§1, same enumeration, item 2); elaborated in the preceding bullet list: "We formulate a new task termed Interactive Mask Correction and construct a benchmark to evaluate the ability to modify preexisting masks. We also propose Progressive Merge as the solution. Progressive Merge carries out morphological analysis of the previous masks and the current predictions to decide where to update and preserve." (§1, "Interactive mask correction" bullet)
- C3: Practical-needs framing — "This work makes interactive segmentation better meet the practical needs, which would be beneficial for both the industrial and academic world." (§1, same enumeration, item 3)
- C4: Headline efficiency number — "The FLOPs of our B0-S1 version is 15 times smaller than the lightest RITM [36], 360 times smaller than FCANet [27]." (§4.2 "Computation analysis")
...
|---|---|---|---|---|
| `ritm-interactive-segmentation` | "RITM [36] hardly removes the mask of the cabin with several negative clicks. Moreover, it causes unexpected changes that are far away from the user clicks." (§1, Fig. 2 caption context) + "RITM [36] and 99%AccuracyNet [10] add the previous mask as network input to make the prediction more robust and accurate. These methods achieve excellent performances, but they suffer from slow inference speed, and they could not deal with preexisting masks." (§2 Related Work) + "we follow RITM [36] to use two conv layers to adjust the channel and scale of the click maps and make feature fusion after the stem layers." (§3.1) + "The coarse segmentation is supervised by Normalized Focal Loss $L_{nfl}$ proposed in RITM [36]." (§3.1 Training supervision) | `extended_by` (already authored on `ritm-interactive-segmentation`'s page, confidence high) | high | FocalClick explicitly inherits RITM's feature-fusion scheme, iterative-click training, and normalized focal loss, then critiques RITM's specific failure on preexisting-mask correction as its own point of departure — textbook extension, not supersession (RITM remains a valid foundational reference for feedforward click-based segmentation without the mask-correction/efficiency goals). Matches the existing `extending: ritm-interactive-segmentation` reverse edge on the live `focalclick` page. |
| `hrnet` | "We select the current SOTA methods HRNet+OCR [38, 46] and SegFormer [42] as representative... for both SegFormer [42] and HRNet [38] as our Segmentor to show the universality of our pipeline." (§3.1 "Coarse segmentation") | `feeds_into` (already authored on `hrnet`'s page, target `focalclick`, confidence high) | high | HRNet+OCR is a named, interchangeable Segmentor backbone component (hrnet18s-S1/S2, hrnet32-S2 variants, Table 3) — compositional lineage, chronological (HRNet 2019 ≤ FocalClick 2022). Matches the existing `fedBy: hrnet` reverse edge. |
| `segformer` | Same quote as above, plus Table 2/3 rows: "Ours-segformerB0-S1/S2", "Ours-segformerB3-S2" as explicit Segmentor variants, e.g. "Ours-segformerB3-S2 ... Large Dataset 1.22 1.26 1.48 3.70 5.84 2.92 4.52" (Table 2). | `feeds_into` (already authored on `segformer`'s page, target `focalclick`, confidence high) | high | SegFormer-B0/B3 (MiT encoder + all-MLP decoder) is reused intact as an alternative Segmentor backbone — same compositional-lineage pattern as HRNet, chronological (SegFormer 2021 ≤ FocalClick 2022). Matches the existing `fedBy: segformer` reverse edge. |
| — (FCANet, not registered in `docs/papers/index.yaml`, no Atlas page) | "The FLOPs of our B0-S1 version is 15 times smaller than the lightest RITM [36], 360 times smaller than FCANet [27]." (§4.2); FCANet also appears as a NoC85/NoC90 comparison baseline (§4.2 "Compared against" list). | `compared_with` | medium | Peer practitioner-choice comparison (efficiency/accuracy baseline), not supersession — no framing of FCANet as obsolete, only a quantitative efficiency gap. No edge can be authored until FCANet (Lin et al., "Interactive Image Segmentation with First-Click Attention") is ingested and registered. |
| — (BRS/f-BRS/CDNet/DIOS/RIS-Net/EdgeFlow/99%AccuracyNet, none registered in `docs/papers/index.yaml`, no Atlas pages) | "DIOS [44] first introduces deep learning into interactive segmentation... BRS [18] first introduces online optimization... f-BRS [35] speeds up the BRS [18]... CDNet [5] introduces self-attention into interactive segmentation..." (§2 Related Work, "Interactive segmentation methods" paragraph); "RIS-Net [23] crop multiple rois only based on click positions... EdgeFlow [14] and 99%AccuracyNet [10] make refinement for the boundary of the primitive predictions. They all get finer results at the cost of larger computations." (§2 Related Work, "Local inference" paragraph) | `compared_with` (peer prior-art lineage, not supersession — several of these are direct algorithmic ancestors of RITM/FocalClick's design choices) | low | Historical positioning of the click-based interactive-segmentation subfield; none of these papers is registered or has an Atlas page today. Flagged for future ingestion rather than actioned here — consistent with the "ingest first" gate used for SETR/PVT/Twins in the SegFormer v2 refresh. |
