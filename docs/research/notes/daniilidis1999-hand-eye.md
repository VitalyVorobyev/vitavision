...
Section: Comparison with Tsai-Lenz (originally deferred — blocked on tsai1989-handeye research note; note updated below since resolved)
...
- Both papers' formulations remain in active practitioner use (neither supersedes the other) — the relation type is `alternative_formulation_of`, not `compared_with` (per CLAUDE.md Rule C, this exact pair is the worked example for the type). Proposed entry on the `tsai-lenz-handeye` page: `{ type: alternative_formulation_of, target: daniilidis-dual-quaternion-handeye, confidence: high }` (symmetric type — author on one side only).
- **Status: resolved.** `docs/research/notes/tsai1989-handeye.md` now exists, both notes are present, and both pages already carry this content: `content/algorithms/tsai-lenz-handeye.md` has the `relations[type=alternative_formulation_of, target=daniilidis-dual-quaternion-handeye, confidence=high]` entry and the `## When to choose Tsai-Lenz over Daniilidis` section; `content/algorithms/daniilidis-dual-quaternion-handeye.md` carries the pointer-back Remarks bullet. Retained below for provenance of the original editorial reasoning.
- Applied Remarks bullet on the Daniilidis page:
...
**Status: resolved.** The page now carries `relations: [{ type: alternative_formulation_of, target: daniilidis-dual-quaternion-handeye, confidence: high }]` in frontmatter (superseding the legacy `comparedWith: [daniilidis-dual-quaternion-handeye]` this note originally referenced) and has the error-propagation Remark in §Remarks ("The decoupled formulation amplifies translation error when the camera baseline between stations is short relative to the target depth; simultaneous rotation-and-translation solvers (Park-Martin on the Lie algebra, Daniilidis dual-quaternion) treat the residual jointly and tend to be more robust under that regime"). This is the correct comparison framing.

Section: Remarks
- **`## When to choose Tsai-Lenz over Daniilidis` subsection is now present** on the page, once the tsai1989-handeye research note was written. Content applied (retained here for provenance):
