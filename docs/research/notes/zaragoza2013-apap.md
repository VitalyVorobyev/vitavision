...
Section: Relationship to DHW and SVA — **status: resolved, and resolved to a different relation type than originally proposed**

This note originally reasoned about DHW and SVA within a peer-comparison (`compared_with`-hosting) framework, and had to stretch the "same year → more general hosts" tiebreaker to a case it wasn't designed for ("APAP is a general method and DHW is a special-case method of narrower scope... by the tiebreaker extended to the older-vs-general case, APAP is the better host") because `related`/`comparedWith` offered no vocabulary for "strictly more general." That stretch was the tell: this is CLAUDE.md **Rule A (supersession is not comparison)**, not a peer choice. Both `gao2011-dual-homography` and `lin2011-svastitching` research notes now exist, and both `content/algorithms/gao-dual-homography-stitching.md` and `content/algorithms/lin-sva-stitching.md` have been authored with `generalized_by` edges to `apap-image-stitching` instead:

- **`gao-dual-homography-stitching.md`** — strong form: `quality: historical` + `{ type: generalized_by, target: apap-image-stitching, confidence: high, caution: "APAP's continuous grid of per-cell homographies subsumes the two-plane parametrisation; the two methods are not peer practitioner choices." }`. APAP's continuous per-cell grid strictly subsumes DHW's two-plane blend (any two-plane scene is a degenerate case of the per-cell field); DHW is preserved for citation lineage only, per Rule A's strong form.
- **`lin-sva-stitching.md`** — weak form: `quality:` omitted, `{ type: generalized_by, target: apap-image-stitching, confidence: medium, caution: "Affine deviation field remains a useful baseline; APAP's projective per-cell grid is more general but not strictly necessary for moderate-parallax planar-scene panoramas." }`. SVA's affine field retains practical relevance as a cheaper baseline under moderate parallax, so it keeps `medium` confidence and no historical badge, per Rule A's weak form.

Both edges are authored on the superseded page (A → B, target = `apap-image-stitching`), consistent with the `generalized_by` direction convention (asymmetric, superseded method points at its generalisation). No entry is needed on `apap-image-stitching.md` itself — the build derives the reverse `generalises` bucket. This supersedes the original proposal to add `comparedWith: [gao2011-dual-homography, lin2011-svastitching]` to the APAP page; that field no longer exists, and even under the current schema a `compared_with` edge on APAP's page would have been the wrong type for a supersession relationship.
