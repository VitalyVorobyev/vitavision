#!/usr/bin/env python3
"""
BRIEF pipeline diagram.

Generates a 6-stage pipeline SVG for content/algorithms/brief.md.
Run:  .venv/bin/python py/generate_brief_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "brief" / "pipeline.svg"
TITLE = "BRIEF descriptor extraction pipeline"
DESC = (
    "6-stage pipeline from a greyscale input image through Gaussian smoothing, "
    "patch extraction at each keypoint, n_d pixel-pair binary tests, packing "
    "into 64-bit words, and Hamming-distance matching via bitwise XOR + popcount."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-brief-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the BRIEF descriptor extraction pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
