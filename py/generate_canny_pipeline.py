#!/usr/bin/env python3
"""
Canny edge detector pipeline diagram.

Generates a 6-stage pipeline SVG for content/algorithms/canny-edge-detector.md.
Run:  .venv/bin/python py/generate_canny_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "canny-edge-detector" / "pipeline.svg"
TITLE = "Canny edge detector pipeline"
DESC = (
    "6-stage pipeline from the input image through Gaussian smoothing, gradient "
    "magnitude and direction, non-maximum suppression along the gradient "
    "direction, hysteresis double-thresholding, to the final thin binary edge map."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-canny-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Canny edge detector pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
