#!/usr/bin/env python3
"""
CCDN checkerboard corner detection network pipeline diagram.

Generates an 11-stage pipeline SVG for content/models/ccdn-checkerboard-detector.md.
Run:  .venv/bin/python py/generate_ccdn_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "ccdn-checkerboard-detector" / "pipeline.svg"
TITLE = "CCDN checkerboard corner detection network pipeline"
DESC = (
    "11-stage pipeline from grayscale input through six convolutional layers "
    "with two stride-1 max-pools, producing a full-resolution response map "
    "that is post-processed by adaptive threshold, 4×4 NMS, and k-means++ "
    "cluster pruning to the final corner set."
)

COLS = 4       # 11 stages → 4+4+3 snake
BOX_W = 2.0
BOX_H = 0.92
H_GAP = 0.40
V_GAP = 0.60
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-ccdn-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the CCDN pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
