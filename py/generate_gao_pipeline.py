#!/usr/bin/env python3
"""
Gao dual-homography stitching pipeline diagram.

Generates a 7-stage pipeline SVG for content/algorithms/gao-dual-homography-stitching.md.
Run:  .venv/bin/python py/generate_gao_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "gao-dual-homography-stitching" / "pipeline.svg"
TITLE = "Gao dual-homography stitching pipeline"
DESC = (
    "7-stage pipeline from SIFT matches through spatial K-means clustering, "
    "per-group RANSAC homography fitting, per-pixel blend weight, MRF seam cut, "
    "content-aware straightening, to the final stitched mosaic."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-gao-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Gao dual-homography pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
