#!/usr/bin/env python3
"""
Geiger chessboard detector pipeline diagram.

Generates a 6-stage pipeline SVG for content/algorithms/geiger-chessboard-detector.md.
Run:  .venv/bin/python py/generate_geiger_pipeline.py
"""
...
STAGES = [
    ...
    "Corner likelihood\n4-quad prototypes\n3 fixed scales",
    "NMS\n(τ_nms = 0.02)",
    "Gradient verification\n32-bin orientation hist\n+ mean shift",
    "Subpixel refine\n11×11 grad-orthogonality\n(Eq. 3)",
    "Structure recovery\ngreedy seed expansion\n(Eq. 6-7)",
    "Per-board corner sets",
]

OUTPUT = REPO_ROOT / "content" / "images" / "geiger-chessboard-detector" / "pipeline.svg"
TITLE = "Geiger chessboard detector pipeline"
DESC = (
    "7-stage pipeline from grayscale image through four-quadrant corner likelihood "
    "at three fixed scales, NMS, gradient-orientation verification, subpixel "
    "refinement, greedy structure recovery, to per-board corner sets."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-geiger-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Geiger chessboard detector pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
