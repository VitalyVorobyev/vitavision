#!/usr/bin/env python3
"""
GP checkerboard enhancement (PyCBD) pipeline diagram.

Generates a 5-stage pipeline SVG for content/algorithms/gp-checkerboard-enhancement.md.
Run:  .venv/bin/python py/generate_gp_checkerboard_pipeline.py
"""
...
STAGES = [
    "Upstream detector\n(boardXY, boardUV,\nunallocated cornersUV)",
    "Stage 1\nIterative allocation\n(GP-guided expansion)",
    "GP retraining\non full allocated set",
    "Stage 2\nOccluded fill-in +\nglobal smoothing",
    "Augmented corner set\n(boardXY, boardUV)\n+ per-corner variance",
]

OUTPUT = REPO_ROOT / "content" / "images" / "gp-checkerboard-enhancement" / "pipeline.svg"
TITLE = "GP checkerboard enhancement (PyCBD) pipeline"
DESC = (
    "5-stage pipeline from upstream detector partial output through iterative "
    "GP-guided corner allocation, GP retraining, occluded fill-in and global "
    "smoothing, to the augmented corner set with per-corner variance."
)

COLS = 3       # 5 stages → 3+2 snake
BOX_W = 2.2
BOX_H = 0.92
H_GAP = 0.40
V_GAP = 0.60
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-gp-checkerboard-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the GP checkerboard enhancement pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
