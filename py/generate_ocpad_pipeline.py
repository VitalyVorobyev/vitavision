#!/usr/bin/env python3
"""
OCPAD occluded checkerboard pattern detection pipeline diagram.

Generates a 9-stage pipeline SVG for content/algorithms/ocpad.md.
Run:  .venv/bin/python py/generate_ocpad_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "ocpad" / "pipeline.svg"
TITLE = "OCPAD occluded checkerboard pattern detection pipeline"
DESC = (
    "9-stage pipeline from candidate corner graph through size and spatial "
    "consistency pre-filtering, quad filter, largest-component selection, "
    "anchor-based BFS ordering, VF2 binary-search subgraph matching, BFS "
    "region growing, to the final graph-to-model mapping."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-ocpad-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the OCPAD pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
