#!/usr/bin/env python3
"""
ORB (Oriented FAST + Rotated BRIEF) runtime pipeline diagram.

Generates a 6-stage pipeline SVG for content/algorithms/orb.md.
Run:  .venv/bin/python py/generate_orb_pipeline.py
"""
...
STAGES = [
    "Greyscale image\n$I$",
    "5-level pyramid\n(scale factor $\\sqrt{2}$,\narea-based decimation)",
    "FAST-9 segment test\nat each level\n(adaptive threshold)",
    "Harris ranking\n$R_H = \\det(M) - k\\,\\mathrm{tr}(M)^2$\n→ top-$N/L$ per level",
    "Intensity-centroid\norientation\n$\\theta = \\mathrm{atan2}(m_{01}, m_{10})$",
    "rBRIEF\nsteered 256-bit descriptor\n(30-bin angle LUT)",
]

OUTPUT = REPO_ROOT / "content" / "images" / "orb" / "pipeline.svg"
TITLE = "ORB runtime pipeline"
DESC = (
    "6-stage pipeline from a greyscale image through a 5-level square-root-two "
    "pyramid, FAST-9 segment test at each level, Harris cornerness ranking to "
    "the top N per level, intensity-centroid orientation assignment, to the "
    "rBRIEF 256-bit binary descriptor steered by a 30-bin angle lookup table."
)

COLS = 3       # 6 stages → 3 + 3 snake
BOX_W = 2.4
BOX_H = 1.05
H_GAP = 0.40
V_GAP = 0.65
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-orb-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the ORB runtime pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
