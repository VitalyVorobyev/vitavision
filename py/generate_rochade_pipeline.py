#!/usr/bin/env python3
"""
ROCHADE chessboard detection pipeline diagram.

Generates a 10-stage pipeline SVG for content/algorithms/rochade.md.
Run:  .venv/bin/python py/generate_rochade_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "rochade" / "pipeline.svg"
TITLE = "ROCHADE chessboard detection pipeline"
DESC = (
    "10-stage pipeline from input image through Scharr gradient, local threshold, "
    "conditional dilation, centreline thinning, saddle detection, clustering, grid "
    "verification, cone filtering, and subpixel refinement to final corners."
)

COLS = 4       # 10 stages → 4+4+2 snake
BOX_W = 2.0    # box width (data units = inches at 1:1)
BOX_H = 0.92   # box height
H_GAP = 0.40   # horizontal gap between boxes
V_GAP = 0.60   # vertical gap between rows
FONT_PT = 12   # ≥12 pt floor (Atlas policy)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-rochade-pipeline"
    ...
    # Box + label per stage; track physical centres
    centers = []
    ...
    for i in range(len(centers) - 1):
        ...
        if r1 == r2:
            ...
            # row transition: vertical arrow downwards
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the ROCHADE pipeline SVG for content/algorithms/rochade.md."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
        type=Path,
        default=OUTPUT,
        help="Output SVG path (default: content/images/rochade/pipeline.svg)",
    )
