#!/usr/bin/env python3
"""
Shu topological grid finding pipeline diagram.

Generates a 6-stage pipeline SVG for content/algorithms/shu-topological-grid.md.
Run:  .venv/bin/python py/generate_shu_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "shu-topological-grid" / "pipeline.svg"
TITLE = "Shu topological grid finding pipeline"
DESC = (
    "6-stage pipeline from Harris corners through Delaunay triangulation, "
    "same-colour triangle merging into quads, topological degree filtering, "
    "geometric aspect-ratio filtering, to flood-fill integer grid coordinate "
    "assignment."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-shu-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Shu topological grid pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
