#!/usr/bin/env python3
"""
Tsai-Lenz hand-eye calibration pipeline diagram.

Generates a 7-stage pipeline SVG for content/algorithms/tsai-lenz-handeye.md.
Run:  .venv/bin/python py/generate_tsai_lenz_pipeline.py
"""
...
STAGES = [
    ...
    "Pair motions\nH_g_ij, H_c_ij",
    "Modified Rodrigues\nP_g_ij, P_c_ij",
    "Stack [P_g+P_c]_x P' = P_c − P_g\nleast squares",
    "P_cg = 2P' / √(1+|P'|²)\nR_cg = R(P_cg)",
    "Stack (R_g − I) T = R_cg T_c − T_g\nleast squares",
    "H_cg = [R_cg | T_cg]",
]

OUTPUT = REPO_ROOT / "content" / "images" / "tsai-lenz-handeye" / "pipeline.svg"
TITLE = "Tsai-Lenz hand-eye calibration pipeline"
DESC = (
    "7-stage pipeline from station observations through pair motion computation, "
    "modified Rodrigues vector encoding, rotation linear least squares, rotation "
    "recovery, translation linear least squares, to the final hand-eye transform."
)
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-tsai-lenz-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Tsai-Lenz hand-eye calibration pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
