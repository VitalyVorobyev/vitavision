#!/usr/bin/env python3
"""
Zhang's planar camera calibration pipeline diagram.

Generates a 7-stage pipeline SVG for content/algorithms/zhang-planar-calibration.md.
Run:  .venv/bin/python py/generate_zhang_pipeline.py
"""
...
OUTPUT = REPO_ROOT / "content" / "images" / "zhang-planar-calibration" / "pipeline.svg"
TITLE = "Zhang's planar camera calibration pipeline"
DESC = (
    "7-stage pipeline from planar correspondence detection through per-view "
    "homography fitting, IAC constraint matrix assembly, SVD null-vector solve, "
    "closed-form intrinsic recovery, per-view extrinsic extraction, to full "
...
def render(stages, output_path: Path, title: str, desc: str, cols: int = COLS) -> None:
    ...
    plt.rcParams["svg.hashsalt"] = "vitavision-zhang-pipeline"
...
def main(argv=None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate the Zhang planar calibration pipeline SVG."
    )
    parser.add_argument(
        "output_path",
        nargs="?",
