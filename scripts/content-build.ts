...
function generateOutput(blogPosts: BlogEntry[], algorithmPages: AlgorithmEntry[], demoPages: DemoEntry[], modelPages: ModelEntry[], conceptPages: ConceptEntry[]): void {
    ...
    const conceptIndex: ConceptIndexEntry[] = conceptPages.map(({ slug, frontmatter }) => ({ slug, frontmatter }));
...
async function main(): Promise<void> {
    ...
    generateOutput(blogPosts, algorithmPages, demoPages, modelPages, conceptPages);

    // Build content graph from all non-draft entries.
    // Use all raw entries (before draft filter) to support INCLUDE_DRAFTS mode,
    // but only include what was actually filtered through to generated output.
    const graphEntries: ContentEntry[] = [
        ...algorithmPages.map((e) => ({
            slug: e.slug,
    ...
    // Build search records from all non-draft entries.
    const searchEntries: SearchEntry[] = [
        ...algorithmPages.map((e) => ({
            ...
            category: (e.frontmatter as { category?: string }).category,
            html: e.html,
        })),
        ...modelPages.map((e) => ({
            slug: e.slug,
