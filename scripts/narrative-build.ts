...
export function resolveNode(
    ...
    if (node.question !== undefined) {
        const out: NarrativeQuestionNode = {
            id: node.id,
            kind: "question",
            title: node.question,
            area: node.area,
...
export function resolveNarrative(
    ...
    const timelineInput: TimelineNodeInput[] = nodes.map((n) => ({
        ...
        year:
            n.kind === "page" ? n.year
            : n.kind === "paper" ? (n.year > 0 ? n.year : undefined)
            : undefined,
        area: n.area,
    }));
...
function renderNarrativeModuleSource(entry: Pick<NarrativeEntry, "html" | "chapters" | "narrative" | "frontmatter">): string {
    const walkthrough: NarrativeWalkthrough = entry.frontmatter.walkthrough ?? "focus";
    return [
        ...
        "",
        // "focus" (dim only) is the default when a narrative omits `walkthrough`.
        `export const walkthrough: "focus" | "reveal" = ${JSON.stringify(walkthrough)};`,
        "",
    ].join("\n");
}
