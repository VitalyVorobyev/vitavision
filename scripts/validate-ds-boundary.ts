...
const result = await build({
    ...
    packages: "external",
    // JSON is a dependency LEAF — it imports nothing, so it can never introduce
    // a forbidden edge and excluding it costs no coverage. Keeping it external
    // means the check needs no build artifacts: ds-entry.tsx seeds PapersContext
    // from public/papers-index.json, which is generated by `content:build` and
    // gitignored, so bundling it for real would make this fail on a bare
    // checkout — and this must run BEFORE the build to be worth having.
    external: ["*.json"],
