    ...
    const handleMouseMove = () => {
        ...
        if (hiddenCanvasRef.current && pos.x >= 0 && pos.x < imageWidth && pos.y >= 0 && pos.y < imageHeight) {
            ...
            if (ctx) {
                ...
                setHoverPixel({ x: px, y: py, r: pixel[0], g: pixel[1], b: pixel[2] });
            }
        } else {
            setHoverPixel(null);
        }

        // 2. Interactive Drawing BBox/Ellipse
        ...
        >
            ...
            {hoverPixel && (
                <div className="absolute top-4 left-4 z-20 bg-background/90 border border-border backdrop-blur p-2 rounded-md shadow-sm text-xs font-mono flex items-center space-x-4 pointer-events-none">
                    <div>
                        <span className="text-muted-foreground">X:</span> {hoverPixel.x} <span className="text-muted-foreground">Y:</span> {hoverPixel.y}
                    </div>
                    <div className="flex items-center space-x-2">
                        <div className="w-3 h-3 rounded-full border border-border" style={{ backgroundColor: `rgb(${hoverPixel.r}, ${hoverPixel.g}, ${hoverPixel.b})` }} />
                        <span>RGB({hoverPixel.r}, {hoverPixel.g}, {hoverPixel.b})</span>
                        <span className="text-muted-foreground">|</span>
                        {/* Simple luminance for grayvalue */}
                        <span>Gray: {Math.round(0.299 * hoverPixel.r + 0.587 * hoverPixel.g + 0.114 * hoverPixel.b)}</span>
                    </div>
                </div>
            )}
