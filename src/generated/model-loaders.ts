// Auto-generated by scripts/content-build.ts — do not edit manually.
export interface GeneratedHtmlModule { html: string; }

export const modelHtmlLoaders: Record<string, () => Promise<GeneratedHtmlModule>> = {
  "alexnet": () => import("./content/models/alexnet.ts"),
  "bisenet": () => import("./content/models/bisenet.ts"),
  "ccdn-checkerboard-detector": () => import("./content/models/ccdn-checkerboard-detector.ts"),
  "ccs-camera-calibration": () => import("./content/models/ccs-camera-calibration.ts"),
  "clip": () => import("./content/models/clip.ts"),
  "deeplab-semantic-segmentation": () => import("./content/models/deeplab-semantic-segmentation.ts"),
  "deit": () => import("./content/models/deit.ts"),
  "depth-anything": () => import("./content/models/depth-anything.ts"),
  "depth-anything-3": () => import("./content/models/depth-anything-3.ts"),
  "depth-anything-v2": () => import("./content/models/depth-anything-v2.ts"),
  "detr": () => import("./content/models/detr.ts"),
  "dino": () => import("./content/models/dino.ts"),
  "dinov2": () => import("./content/models/dinov2.ts"),
  "dinov3": () => import("./content/models/dinov3.ts"),
  "dpt": () => import("./content/models/dpt.ts"),
  "dust3r": () => import("./content/models/dust3r.ts"),
  "efficientad": () => import("./content/models/efficientad.ts"),
  "fast-scnn": () => import("./content/models/fast-scnn.ts"),
  "faster-rcnn": () => import("./content/models/faster-rcnn.ts"),
  "fcn-semantic-segmentation": () => import("./content/models/fcn-semantic-segmentation.ts"),
  "fpn": () => import("./content/models/fpn.ts"),
  "focalclick": () => import("./content/models/focalclick.ts"),
  "googlenet": () => import("./content/models/googlenet.ts"),
  "hrnet": () => import("./content/models/hrnet.ts"),
  "lightglue": () => import("./content/models/lightglue.ts"),
  "loftr": () => import("./content/models/loftr.ts"),
  "mae": () => import("./content/models/mae.ts"),
  "mask-rcnn": () => import("./content/models/mask-rcnn.ts"),
  "mask2former": () => import("./content/models/mask2former.ts"),
  "mast3r": () => import("./content/models/mast3r.ts"),
  "mate-checkerboard-detector": () => import("./content/models/mate-checkerboard-detector.ts"),
  "midas": () => import("./content/models/midas.ts"),
  "mnasnet": () => import("./content/models/mnasnet.ts"),
  "mobilenetv2": () => import("./content/models/mobilenetv2.ts"),
  "mobilenetv3": () => import("./content/models/mobilenetv3.ts"),
  "mobilesam": () => import("./content/models/mobilesam.ts"),
  "patchcore": () => import("./content/models/patchcore.ts"),
  "pointrend": () => import("./content/models/pointrend.ts"),
  "raft": () => import("./content/models/raft.ts"),
  "resnet": () => import("./content/models/resnet.ts"),
  "rf-detr": () => import("./content/models/rf-detr.ts"),
  "ritm-interactive-segmentation": () => import("./content/models/ritm-interactive-segmentation.ts"),
  "sam": () => import("./content/models/sam.ts"),
  "segformer": () => import("./content/models/segformer.ts"),
  "superglue": () => import("./content/models/superglue.ts"),
  "superpoint": () => import("./content/models/superpoint.ts"),
  "swin": () => import("./content/models/swin.ts"),
  "unet-segmentation": () => import("./content/models/unet-segmentation.ts"),
  "uninformed-students": () => import("./content/models/uninformed-students.ts"),
  "vgg": () => import("./content/models/vgg.ts"),
  "vggt": () => import("./content/models/vggt.ts"),
  "vit": () => import("./content/models/vit.ts"),
  "xfeat": () => import("./content/models/xfeat.ts"),
  "yolo-v1": () => import("./content/models/yolo-v1.ts"),
};
